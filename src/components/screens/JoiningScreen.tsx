...
interface JoiningScreenProps {
  ...
  setCustomAudioStream: React.Dispatch<React.SetStateAction<MediaStream | null>>;
  setCustomVideoStream: React.Dispatch<React.SetStateAction<MediaStream | null>>;
}

const JoiningScreen:React.FC<JoiningScreenProps> = ({
  participantName,
  setParticipantName,
  setMeetingId,
  setToken,
  ...
  const permissonAvaialble = useRef<{ isCameraPermissionAllowed: boolean, isMicrophonePermissionAllowed: boolean } | null>(null);
  const webcamRef = useRef<boolean | null>(null);
  const micRef = useRef<boolean | null>(null);
  ...
  async function requestAudioVideoPermission(mediaType: string) {
    try {
  ...
  return (
    <div className="fixed inset-0">
      <div style={{backgroundColor: '#FEFAF6'}} className="overflow-y-auto flex flex-col flex-1 h-screen bg-gray-800">
